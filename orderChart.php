  ...
  <div class="wrapper">
    ...
    <div class="main-panel">
	  ...
	  <div class="content">
		...
		$sql = "SELECT MIN(`pricemax`) AS 'min',MAX(`pricemax`) AS 'max' FROM (SELECT `pricemax` FROM `".strtolower($symbol)."` ORDER BY `id` DESC LIMIT ".$_GET['long']." OFFSET ".$_GET['start']." ) AS fff ";
		...
		//all price of chart
		$sql = "SELECT * FROM (SELECT `id`,`time`,`pricemax`,`peak` FROM `".strtolower($symbol)."` ORDER BY `id` DESC LIMIT ".$_GET['long']." OFFSET ".$_GET['start'].") AS AAA ORDER BY `id` ASC";
		...
		echo "hi";
		$levelSpace = round(($maxChart-$minChart)/50,3); // faseleie level ha samte chap
		echo $levelSpace."<br>";
		$numOfLevels = 50;
		$i=0;
		while($numOfLevels > $i){
		...
		*/
		...
		function fiAv($price,$num,$len,$fiSeri){
			//echo($price[-2]."</br>");
			$sum =0;
			$sum2 =0;
			$sumW =0;
			...
			for($i=0;$i<$num;$i+=1){
				for($j=0;$j<$len;$j+=1){
					$sum2 = $sum2 + $price[$i*$len + $j]*$fiSeri[$j];
					//echo ($i*$len + $j)."/".$price[$i*$len + $j]."</br>";
					$sum2W = $sum2W + $fiSeri[$j];
				}
				$priceAv[$i] = $sum2 / $sum2W;
				$sum2W = 0;
				$sum2 = 0;
			}
			for($i=0;$i<$num;$i=$i+1){
				$sum += ($priceAv[$num-1] - $priceAv[$i])*$fiSeri[$i];
				$sumW += $fiSeri[$i];
			}
			return round($sum/$sumW,2);
		}
		//fiAv($arrayPrice,$fiNum,$fiLen,fiSeri($fiNum))
		$fiLen = 5 ; //per min
		$fiNum = 15 ; // ta 30 ta zarib fibunachi estefade shavad
			
		
		$kk = 0; // counter for loop
		$row = $results->fetch_assoc();
		$smallestID = $row['id'];
		do {
			
			//$level[intdiv(($row['pricemax'] - $minChart) / $levelSpace,1)] += 1; // ezafe kardan iek vahed be har level
			...
			*/
			...
			if($row['peak'] == 1 or $row['peak']==-1){
				array_unshift($dataPointspeak, array("x" => $row['id'],"label" => $row['peak']."id:".$row['id'] ,"y" => $row['pricemax']));
			}
			
				/*
			$fi = fiAv($arrayPrice,$fiNum,$fiLen,fiSeri($fiNum));
			$fi2 = fiAv($arrayPrice,15,4,fiSeri($fiNum));
			if($fi < $fi2 ){
						array_unshift($dataPointsMV, array("x" => $row['id'],"label" => $fi ,"y" => $minChart));
				}else{
						array_unshift($dataPointsMV, array("x" => $row['id'],"label" => $fi."-".$fi2 ,"y" => $maxChart));
				}
				
			*/
			
			//fibonachi
			//array_push($arrayPrice,$row['pricemax']);
			//
			//if(count($arrayPrice) >  $fiNum*$fiLen){
			//	$arrayPrice=array_slice($arrayPrice,1);
			//	// echo "-".count($arrayPrice)."-".$arrayPrice[-1];
			//	$fi = fiAv($arrayPrice,$fiNum,$fiLen,fiSeri($fiNum));
			//	$kk += $fi;
			//	//array_unshift($dataPoints2, array("x" => $row['id'],"label" => $fi."-".$row['pricemax'] ,"y" => 16202.1+$fi*10));
			//	//array_unshift($dataPoints2, array("x" => $row['id'],"label" => $fi."-".$row['pricemax'] ,"y" => $kk/1600*223+16000));
			//	
			//	// if($fi < -7 ){
			//	// array_unshift($dataPointsMV, array("x" => $row['id'],"label" => $fi."-".$row['pricemax'] ,"y" => $minChart-50));
			//	// }elseif($fi > 7){
			//	// array_unshift($dataPointsMV, array("x" => $row['id'],"label" => $fi."-".$row['pricemax'] ,"y" => $maxChart+50));
			//	// }
			//}
			$kk += 1;
			...
			if(fmod($i,$interval) == 0){
				...
				*/
				...
				if($fi < -5 ){
						...
						array_unshift($dataPointsMV, array("x" => $row['id'],"label" => $fi ,"y" => $maxChart));
				}
				*/
				...
				array_unshift($dataPoints, array("x" => $row['id'],"label" => $kk ,"y" => $minOfInterval));
				//array_unshift($dataPoints, array("x" => $row['id'],"label" => "id:".$row['id'],"y" => $maxOfInterval));
				array_unshift($dataPoints, array("x" => $row['id'],"label" => $kk,"y" => $maxOfInterval));
		...
		while($numOfLevels >= $i){
			${'level'.$i} = array();
			array_unshift(${'level'.$i}, array("label" => $level[$i],"x" => $smallestID  ,"y" => $minChart + $i*$levelSpace));
			array_unshift(${'level'.$i}, array("x" => $smallestID  + $level[$i] ,"y" => $minChart + $i*$levelSpace));
			$i += 1;
		}

		$sql = "SELECT * FROM `orders` WHERE symbol = '$symbol' ORDER BY `status` DESC ,`price` DESC ";
		$results = $conn->query($sql);
		...
		while ( $row = $results->fetch_assoc()){
		if ($row['price']==0){$row['price']=$row['pricefill'];}
		if($_GET['long']>250000 or (($row['price'] > ($minChart*(1-$_GET['long']/2500000)) and $row['price'] < ($maxChart*(1+$_GET['long']/2500000)) ) )){	
