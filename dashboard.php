    ...
    <div class="main-panel">
	  ...
	  <div class="content">
		...
		*/
		//nemoodar dovom
		$datatotalbtc = array();
		$sql = "SELECT totalbtc.totalbtc, totalbtc.id_btcusdt,totalbtc.time,btcusdt.pricemax FROM `totalbtc` INNER JOIN btcusdt ON totalbtc.id_btcusdt = btcusdt.id ORDER BY totalbtc.id DESC LIMIT 5000";
		$results = $conn->query($sql);
		while ( $row = $results->fetch_assoc()){
		...
		//$sql = "SELECT * FROM `orders` WHERE `status` = 3 AND `id_fill` > (SELECT MIN(`id_btcusdt`) FROM `totalbtc`) ORDER BY `id` ASC";
		//$results = $conn->query($sql);
		//while ( $row = $results->fetch_assoc()){
